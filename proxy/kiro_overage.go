...
func FetchOverageStatus(account *config.Account) (*OverageSnapshot, error) {
	...
	rawURL := kiroManagementAPIBase(account) + "/getUsageLimits?origin=AI_EDITOR&resourceType=AGENTIC_REQUEST&isEmailRequired=true"
...
func SetOverageStatus(account *config.Account, enabled bool) (*OverageSnapshot, error) {
	...
	req, err := http.NewRequest("POST", kiroManagementAPIBase(account)+"/setUserPreference", bytes.NewReader(body))
	if err != nil {
		return nil, err
	}
	setKiroHeaders(req, account)
