...
func ClaudeToKiro(req *ClaudeRequest, thinking bool) *KiroPayload {
	...
	// Build final content. Orphaned tool results cannot stay structured, but
	// their text must still survive alongside any attached images or user text.
	finalContent := ""
	if !keepCurrentToolResults && len(currentToolResults) > 0 {
		finalContent = joinHistoryText(currentContent, narrateToolResults(currentToolResults, nil))
...
func extractClaudeDocumentSourceText(block, source map[string]interface{}) string {
	sourceType := strings.ToLower(firstMapString(source, "type"))
	mediaType := firstMapString(source, "media_type", "mime_type", "mime")
	if mediaType == "" {
		mediaType = firstMapString(block, "media_type", "mime_type", "mime")
	}
	data := firstMapString(source, "data", "base64", "content")
	if data == "" {
		return ""
	}
	...
	case "base64", "":
		if parsedMediaType, decoded, ok := decodeClaudeDocumentData(data); ok {
			...
			if text := decodeClaudeDocumentBytes(decoded, mediaType); text != "" {
				return formatClaudeDocumentText(block, text)
			}
		}
		if isTextDocumentMediaType(mediaType) {
			return formatClaudeDocumentText(block, data)
		}
	}
	return ""
	...
	return &ClaudeResponse{
		...
		Usage: ClaudeUsage{
			...
			OutputTokensDetails: map[string]int{
				"thinking_tokens": estimateApproxTokens(thinkingContent),
			},
		},
...
func OpenAIToKiro(req *OpenAIRequest, thinking bool) *KiroPayload {
	...
	// Build final content. Preserve orphaned tool output as text because Kiro
	// only accepts structured tool results for the active final tool turn.
	finalContent := currentContent
	if !keepCurrentToolResults && len(currentToolResults) > 0 {
		finalContent = joinHistoryText(finalContent, narrateToolResults(currentToolResults, nil))
	}
	if finalContent == "" {
